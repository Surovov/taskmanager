...
// Подсчет общего количества архивированных задач пользователя
try {
    $stmt = $pdo->prepare('SELECT COUNT(*) FROM tasks WHERE user_id = :user_id AND status = 3');
...
// Получение архивированных задач пользователя
try {
    $stmt = $pdo->prepare('SELECT * FROM tasks WHERE user_id = :user_id AND status = 3 LIMIT :offset, :tasksPerPage');
    $stmt->bindParam(':user_id', $_SESSION['user_id'], PDO::PARAM_INT);
